...
class TelegramMonitor:
    ...
    async def send_to_webhook(self, lead_data):
        """Отправляет лид в n8n"""
        try:
            # Отправляем в n8n
            webhook_url = "https://your-n8n-instance.com/webhook/telegram-leads"  # ЗАМЕНИТЕ на ваш n8n webhook
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        print(f"✅ Лид отправлен в n8n (всего: {self.total_leads_found})")
                        return True
                    else:
                        print(f"❌ Ошибка отправки в n8n: {response.status}")
                        return False
...
def webhook_parser():
    """Webhook для тестирования из n8n"""
    data = request.get_json()
    print("✅ Получены данные из n8n:", data)
    
    return jsonify({
        "status": "success",
        "message": "Data received from n8n",
        "received_data": data
    })
